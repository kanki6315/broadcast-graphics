 ...
 */
export function LinearTrackRibbon({ drivers, selectedCarIdx, nearbyCarIdxs = new Set(), onSelectCar, variant = "control" }: LinearTrackRibbonProps) {
  ...
  return (
    <section className={`track-ribbon variant-${variant}`} aria-label="Linear track position">
      ...
      <div className="track-ribbon-pits">
        ...
        <div>{inPits.map((driver) => <button className={`${driver.carIdx === selectedCarIdx ? "is-selected" : ""}${nearbyCarIdxs.has(driver.carIdx) ? " is-nearby" : ""}`} key={driver.carIdx} type="button" onClick={() => onSelectCar(driver.carIdx)} style={{ "--class-color": driver.classColor } as CSSProperties}>#{driver.carNumber}</button>)}</div>
      </div>
      {unobserved.length > 0 && <div className="track-ribbon-unobserved"><span className="track-ribbon-label">Unknown</span><div>{unobserved.map((driver) => <button className={driver.carIdx === selectedCarIdx ? "is-selected" : ""} key={driver.carIdx} type="button" onClick={() => onSelectCar(driver.carIdx)} style={{ "--class-color": driver.classColor } as CSSProperties}>#{driver.carNumber}</button>)}</div></div>}
    </section>
  );
}
