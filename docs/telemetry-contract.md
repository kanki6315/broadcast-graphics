...
`cameraGroups`, `activeCameraCarIdx`, `activeCameraGroup`, and `activeCamera` report the current session's available camera inventory and observed camera selection. They are optional so older diagnostic captures remain replay-compatible. Camera changes still travel through the separate command path: `focus.set`, `camera.group.take`, and the compatibility `camera.take` action produce a server-to-client `camera.command`; the telemetry client returns a `camera.result` after handing the request to the SDK.
