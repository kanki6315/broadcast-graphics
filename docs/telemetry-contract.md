...
The optional session `sectorDefinition` normally comes from `SessionInfo.SplitTimeInfo.Sectors`. The Windows client orders the variable-length `SectorNum`/`SectorStartPct` list, requires a usable start/finish boundary, associates it with the session and track identity, and hashes those facts into a stable `iracing-…` revision. It does not invent boundaries when the SDK definition is absent or unusable. Before racing, an administrator may activate an immutable custom revision for the exact layout. The server delivers that compact definition to the telemetry client with the low-frequency `sector.definition` message; no map geometry is included. A definition change resets the local lifecycle and the shared intelligence cache, so results from different revisions never share rankings.

Each optional `DriverState.sectors` contains the current sector number, completed sectors for the current and previous lap, and the car's fastest valid result for each sector in `bestSectors`. A completed sector includes lap, sector number, elapsed value when available, revision, completion/observation session time, source, quality, invalidity reason, and valid-only comparison labels. Only `valid` derived sectors can become personal, class, or overall fastest or support pace comparisons. Before a complete set is accepted, its sum must reconcile with the official completed lap within `max(0.5 seconds, 1% of the official lap time)`, which accommodates sampling/interpolation error without accepting a definition mismatch.
...
For calibration `c` and lap percentage `p`, forward travel maps to `wrap(c.startFinishPathPct + p)` of SVG path length; reverse travel maps to `wrap(c.startFinishPathPct - p)`. The inverse operation projects a pointer expressed in the SVG view box to the nearest path distance, then removes the start offset and reverses the sign when required. Screen pixels are never stored as sector percentages.

Only connected circulating cars with finite, non-invalid `lapDistPct` are eligible for centerline placement. Inferred progress is explicitly styled as uncertain. Pit-lane/stall cars are kept in a pit dock unless a separately calibrated pit path exists; unobserved, retired, not-in-world, and invalid-position cars are counted in the unavailable dock. The linear ribbon remains the fallback for missing/mismatched maps, absent calibration, validation/load/render failure, and user preference.

Imported SVG is untrusted. The server rejects scripts, event attributes, external references, embedded HTML/media, style/font/image elements, document entities, unsafe `url()` usage, oversized documents, excessive paths/elements/commands, non-finite coordinates, extreme coordinate bounds, zero-length paths, and open centerlines outside the documented tolerance. Stored markup contains only an `<svg viewBox>` and validated path `id`/`d` values.
